...
const HiddenItem = (props) => {
  ...
  //  border: answered ? '1px solid black' : 'none',
    ...
    ...(children ? {} : lilStyle), 
  } } onClick={e => {if(!children) c(id); if(answered) {e.stopPropagation()}}}></div>
  <span 
   label={id}
    style={children ? {
    //  border: answered ? '1px solid black' : 'none',
     ...
     onClick={e => {if(children) c(id); if(answered) {e.stopPropagation()}}}>
...
<p>{'🐸'.repeat(totalCorrected)}</p>
<p style={{color: totalCorrected === 10 ? 'green' : 'black' }}>
  <b>{totalCorrected}</b>/10 corrected {totalCorrected === 10 ? '👏' : ''} with <b>{attempts}</b> click{attempts === 1 ? '' : 's'} for a score of <b style={{color: score >= 0 ? 'green' : 'red'}}>{score}</b> {score === 100 ? <b style={{color: 'gold'}}>Perfect score! 🥳</b> : score > 0 ? '🤙' : <></>}
</p>
